...
export const HedgeUniV3Plot = (props: any) => {
  ...
  const { isHasDerionIndex, isUpdatingCurrentIndex } = useMemo(() => {
    const isHasDerionIndex = findMatchingPoolIndex() !== null
    const isCurrentIndexMatch = findMatchingPoolIndex(id) !== null
    if (ignoreMatchIndex && !isHasDerionIndex) {
      return {
        isHasDerionIndex: true,
        isUpdatingCurrentIndex: false
      }
    }
    return { isHasDerionIndex: isHasDerionIndex, isUpdatingCurrentIndex: !!(isHasDerionIndex && !isCurrentIndexMatch) }
      ...
      <Card className='p-1 plot-chart-box flex flex-col justify-center items-center pb-[80px] pt-[80px] gap-6'>
        {isUni3Loading || isUpdatingCurrentIndex ? ''
          ...
          >
            ...
            <Expression id='Hedge-H' latex='(L_{s}, D_{s})' showLabel color='RED' label='H' pointOpacity={2} pointSize={40} />
            ...
            <Expression id='IL-A-function' latex={'\\left(x_{a},i\\left(x_{a}\\right)\\right)'} dragMode='NONE' color='GREEN' showLabel label={`${hedgeData?.disPxA}`} />
            <Expression id='IL-B-function' latex={'\\left(x_{b},i\\left(x_{b}\\right)\\right)'} dragMode='NONE' color='PURPLE' showLabel label={`${hedgeData?.disPxB}`} />
            <Expression id='IL-X-function' latex={'\\left(X,i\\left(X\\right)\\right)'} color='BLACK' dragMode='NONE' showLabel label={`${hedgeData?.disPx}`} />
            <Expression id='IL-iHxa-function' latex={'\\left(x_{a},i_{H}\\left(x_{a}\\right)\\right)'} dragMode='NONE' color='GREEN' />
            <Expression id='IL-iHxb-function' latex={'\\left(x_{b},i_{H}\\left(x_{b}\\right)\\right)'} dragMode='NONE' color='PURPLE' />
